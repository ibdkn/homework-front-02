...
function Clock() {
    ...
    const monthOptions: any = { month: 'long' }
    const dayOptions: any = { weekday: 'long' }
    ...
    const start = () => {
        ...
        if (!timerId) { // Если таймер не запущен
            ...
            setTimerId(id as unknown as number); // Сохранить ID таймера
        }
    }
    ...
    const stringTime = date.toLocaleTimeString('en-GB', timeOptions) || <br/> // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
    const stringDate = date.toLocaleString('en-GB', dateOptions).replace(/\//g, '.'); <br/> // день.месяц.год (01.02.2022) // пишут студенты, варианты 01.02.0123/01.02.-123/01.02.12345 не рассматриваем
